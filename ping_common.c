...
/* daveti: function used to clear the ARP cache */
static int ncping_clear_arp_cache(int arp_sock, char *ip)
{
        int rtn;
        ...
        strncpy(arpReq.arp_dev, NCPING_ARP_SOCK_IF_NAME, sizeof(arpReq.arp_dev)-1);

        /* Issue the ARP request to the kernel */
        rtn = ioctl(arp_sock, SIOCDARP, &arpReq);
        if (rtn < 0)
        {
                fprintf(stderr, "Error: ncping delete ARP entry [%s] failure: %s\n", ip, strerror(errno));
		return -1;
        }
...
//daveti: update the main_loop
//void main_loop(int icmp_sock, __u8 *packet, int packlen)
void main_loop(int icmp_sock, __u8 *packet, int packlen, int arp_sock, char *ip)
{
	char addrbuf[128];
	char ans_data[4096];
	struct iovec iov;
...
int dd = 0;
	...
	for (;;) {
		...
		do {
//daveti: hacking here
//will clear the ARP cache before each ping
//sleep at first
			sleep(5);
			next = ncping_clear_arp_cache(arp_sock, ip);
			if (next != 0)
				printf("Error: ncping unable to clear the ARP cache for IP [%s]\n", ip);
			//daveti: shall we sleep for a sec?
